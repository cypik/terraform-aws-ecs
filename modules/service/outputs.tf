...
output "id" {
  ...
  value       = try(aws_ecs_service.main[0].id, aws_ecs_service.ignore_task_definition[0].id, null)
}

output "name" {
  description = "Name of the service"
  value       = try(aws_ecs_service.main[0].name, aws_ecs_service.ignore_task_definition[0].name, null)
}
...
output "task_set_id" {
  ...
  value       = try(aws_ecs_task_set.main[0].task_set_id, aws_ecs_task_set.ignore_task_definition[0].task_set_id, null)
}

output "task_set_arn" {
  description = "The Amazon Resource Name (ARN) that identifies the task set"
  value       = try(aws_ecs_task_set.main[0].arn, aws_ecs_task_set.ignore_task_definition[0].arn, null)
}

output "task_set_stability_status" {
  description = "The stability status. main indicates whether the task set has reached a steady state"
  value       = try(aws_ecs_task_set.main[0].stability_status, aws_ecs_task_set.ignore_task_definition[0].stability_status, null)
}

output "task_set_status" {
  description = "The status of the task set"
  value       = try(aws_ecs_task_set.main[0].status, aws_ecs_task_set.ignore_task_definition[0].status, null)
}
...
output "autoscaling_scheduled_actions" {
  ...
  value       = aws_appautoscaling_scheduled_action.main
}
